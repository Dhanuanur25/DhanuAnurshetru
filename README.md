# Dhanu Anurshetru

🎓 MS in Business Analytics @ University at Buffalo (GPA: 3.93)  
💼 4 years in Data Analytics | SQL | Python | Tableau | Power BI  
📍 Based in Buffalo, NY | Open to Data Analyst & BI Opportunities  

---

## ⚙️ Tech Toolbox

**🧠 Languages & Scripting**: Python | R | SQL | Shell Scripting  
**🗃️ Databases & Warehousing**: Oracle | MySQL | Snowflake | BigQuery  
**🧰 ETL & Workflow Tools**: Apache Hop | dbt | AWS Glue | Airflow  
**📊 BI & Visualization**: Power BI | Tableau | Excel  
**📚 Data Science Libraries**: pandas | NumPy | seaborn | matplotlib | scikit-learn | OpenAI | LlamaIndex  
**☁️ Cloud & Dev Tools**: Oracle Cloud | AWS | Streamlit | FastAPI  
**🛠️ IDEs & Utilities**: Jupyter | SQL Developer | DBeaver | Linux
 

---

## 💼 Work Experience

### 🏥 Project Analyst Intern, University at Buffalo  
*Jan 2025 – May 2025*  
- ⏱️ Reduced physician lookup time by 86% with a smart query-based search tool.  
- 💸 Built an Excel-based cost-benefit model for 12+ surgeons, forecasting $50K–$150K in potential uplift.  
- 🌽 Designed Power BI dashboards for a local farm, boosting repeat visits by 25%.

### 📡 Data Analyst, Subex  
*Aug 2021 – May 2024*  
- 🧾 Analyzed 10M+ telecom records to flag anomalies, reduce incidents, and save $1M+ annually.  
- 📊 Created automated SQL pipelines and visual dashboards for fraud detection and reporting.  
- 🏆 Awarded “Agile Surfer” for slashing aged ticket backlog across 5 European clients.

---

## 📂 Featured Projects

| Project | Tech Stack | Description |
|--------|------------|-------------|
| **📈 Amazon Revenue Trends** | Tableau | Dual-axis dashboards analyzing seasonal and category-wise sales across regions. |
| **🏪 Retail Data Warehouse** | Oracle, Apache Hop, Tableau | End-to-end ETL with SCD logic, automated reporting replacing manual Excel workflows. |
| **🐾 BuddyAI – Pet Wellness Chatbot** | Streamlit, FastAPI, ChromaDB, LLaMA 3 | RAG-based assistant offering real-time answers to pet care questions (cats & dogs). |
| **📊 Buffalo Prep** *(private)* | Excel, Power BI | Alumni outcome analysis with GPA uplift and program impact insights. |
| **🧮 UBMD Acute Care** *(private)* | Excel, VBA | Surgeon-specific cost-benefit modeling and UI-based search optimization. |

> ⚠️ Some academic/industry projects are not public due to data sensitivity.

---

## 🎓 Education

**MS, Business Analytics**  
*University at Buffalo, SUNY (Jul 2024 – May 2025)*  

**BE, Computer Science**  
*Visvesvaraya Technological University (2017 – 2021)*

---

## 📬 Let's Connect  
📧 dhanuanur25@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/dhanu-anurshetru-230285199)  
📂 [Project Portfolio](https://github.com/Dhanuanur25)

---

<!--
![Dhanu's GitHub stats](https://github-readme-stats.vercel.app/api?username=Dhanuanur25&show_icons=true&theme=radical)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Dhanuanur25&layout=compact&theme=radical)
-->
